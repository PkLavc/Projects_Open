...
Um corpo de 3,0 kg está em repouso em um colchão de ar horizontal de atrito
desprezível quando uma força horizontal constante F é aplicada, fazendo-o se
deslocar por 20 metros com uma velocidade de 3m/s. Qual é a força que foi aplicada
ao corpo?
''') # exercicio 3
...
Uma caixa de ferramentas é empurrada por 2 m em um piso sujo de óleo (sem atrito)
de uma oficina de automóveis. A figura mostra o trabalho T realizado sobre a caixa
pela força horizontal constante em função da posição d da caixa. A escala vertical do
gráfico é definida por Ts = 6,0 J.
...
Um trenó e seu ocupante, com massa total de 85 kg, descem uma encosta e
atingem um trecho horizontal retilíneo com uma velocidade de 37 m/s. Se uma
força desacelera o trenó até o repouso a uma taxa constante de 2,0 m/s{ao_quadrado},
determine:
...
A figura mostra três forças aplicadas a um baú que se
desloca 3 m para a esquerda em um piso sem atrito. Os
módulos das forças são F{um_para_baixo} = 5 N, F{dois_para_baixo} = 9 N, e F{tres_para_baixo} = 3 N; o
...
Dois meninos estão brincando com uma bola de futebol de massa igual a 410 g.
Um deles joga a bola e acerta uma vidraça. Sabendo que a vidraça se encontra a
uma altura de 3,0 m do solo, qual o valor da energia potencial da bola ao atingir
a vidraça? Considere o valor da gravidade local igual a 9,8 m/s{ao_quadrado}.
''') # exercicio 2
...
Se, sob pânico, ela efetivamente suspendeu 4000N (cerca de {um_quarto} do peso do veículo) por 5cm, quanto
trabalho a sua força realizou sobre o carro?
''') # exercicio 4

print('''
Exercício 5

No rótulo de uma lata de leite em pó está escrito: valor energético
1509J por 100 g (361 kcal). Se toda energia armazenada em uma lata
que contém 400 g de leite for utilizada para levantar um objeto de
massa 10 kg, qual seria a altura aproximada atingida por este objeto?
a) 25 cm
b) 15 m
c) 400 m
d) 2km
...
Uma corda passa por duas roldanas de massa e atrito
desprezíveis. Uma caixa de 20kg está pendurada em uma
das roldanas. Se for exercida uma força F sobre a
...
Um bloco de gelo de 45kg desce deslizando por um plano
inclinado liso de 1,5m de comprimento e 91cm de altura.
Um trabalhador aplica uma força para cima contra o bloco
de gelo na direção paralela ao plano inclinado, para que o
bloco desça deslizando com velocidade constante.
...
Você deixa cair um livro de 2 kg para uma amiga que está
na calçada, a uma distância D = 10 m abaixo de você. Se as
mãos estendidas da sua amiga estão a uma distância d =
1,5 m acima do solo,
a) qual é o trabalho realizado sobre o livro pela força
gravitacional até o livro cair nas mãos da sua amiga?
b) Se a energia potencial gravitacional U do sistema é
considerada nula no nível do solo, qual é o valor de U
    i. quando você deixa cair o livro
    ii. quando o livro chega às mãos da sua amiga?
c) Suponha agora que o referencial de energia é 100 J ao
nível do solo. Recalcule
    i. o trabalho realizado sobre o livro pela força
    gravitacional até o livro cair nas mãos da sua
    amiga
    ii. A energia potencial gravitacional do livro nas
    mãos de sua amiga
''') # exercicio 8
...
Um bloco de 250 g é deixado cair em uma mola vertical, inicialmente relaxada, cuja constante elástica é k =
2,5 N/cm. O bloco fica acoplado à mola, comprimindo-a em 12 cm até parar momentaneamente. Nessa
...
Calcule a energia potencial elástica armazenada em uma mola, cuja constante elástica é 100 N/m, que está
...
Quando um gatilho é desarmado, a mola da figura, que está
comprimida de 20cm, lança a esfera de massa 0,4kg para a direita.
